  ...
  if (loading) {
    return (
      <div className="fixed inset-0 bg-black flex items-center justify-center z-50">
        <motion.div animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 1, ease: "linear" }} className="text-amber-400 text-5xl">
          <FaSpinner />
        </motion.div>
      </div>
    );
    ...
    <DataContext.Provider value={data}>
      <div className="bg-black text-gray-200 font-sans overflow-x-hidden">
        ...
        <div className="fixed inset-0 z-0">
          ...
          <motion.div animate={{ x: [0, 100, 0], y: [0, 50, 0] }} transition={{ repeat: Infinity, duration: 20, ease: "linear" }} className="absolute top-20 left-10 w-64 h-64 bg-amber-500/5 rounded-full blur-3xl" />
          <motion.div animate={{ x: [0, -80, 0], y: [0, -40, 0] }} transition={{ repeat: Infinity, duration: 15, ease: "linear" }} className="absolute bottom-20 right-10 w-80 h-80 bg-amber-600/5 rounded-full blur-3xl" />
        </div>

        {/* Navigation - FIXED with proper container */}
        <nav className="fixed top-0 left-0 w-full z-50 bg-black/95 backdrop-blur-md border-b border-amber-500/20">
          <div className="max-w-7xl mx-auto w-full px-4 md:px-6 lg:px-8 py-4 flex justify-between items-center">
            ...
            >
              ...
              <motion.div className="absolute -bottom-1 left-0 w-0 h-0.5 bg-amber-400 group-hover:w-full transition-all duration-300" />
            </motion.div>
          ...
          <AnimatePresence>
            {isMenuOpen && (
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="md:hidden bg-black/95 backdrop-blur-md border-b border-amber-500/20">
                <div className="flex flex-col items-center py-4 space-y-4">
                  {["home", "about", "skills", "expertise", "projects", "journey", "education", "contact"].map((item) => (
                    <button key={item} onClick={() => scrollToSection(item)} className={`capitalize text-lg ${activeSection === item ? "text-amber-400" : "text-gray-300"}`}>{item}</button>
                  ))}
    ...
    <section id="home" className="relative min-h-screen flex items-center justify-center pt-20 overflow-hidden">
      <div className="max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-20 text-center relative z-10">
        <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
          <motion.div animate={{ y: [0, -10, 0] }} transition={{ repeat: Infinity, duration: 3, ease: "easeInOut" }} className="inline-block">
            <div className="relative">
              <div className="absolute inset-0 bg-gradient-to-r from-amber-400 to-amber-600 rounded-full blur-2xl opacity-60 animate-pulse"></div>
              <img src={personal.avatar} alt={personal.name} className="w-80 h-80 rounded-full mx-auto mb-6 border-4 border-amber-500 shadow-2xl relative z-10 object-cover" />
            </div>
          </motion.div>
          ...
          <p className="text-lg text-gray-400 max-w-2xl mx-auto mb-8">{personal.bio}</p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => scrollToSection("contact")} className="px-8 py-3 bg-gradient-to-r from-amber-500 to-amber-600 rounded-full font-semibold text-black shadow-lg hover:shadow-amber-500/50 transition-all flex items-center justify-center gap-2 group">
              Hire Me <FaArrowRight className="group-hover:translate-x-1 transition-transform" />
            </motion.button>
            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => scrollToSection("projects")} className="px-8 py-3 border-2 border-amber-500 rounded-full font-semibold text-amber-400 hover:bg-amber-500/10 transition-all">
              Explore Work
            </motion.button>
          </div>
          <div className="grid grid-cols-3 gap-6 mt-16 max-w-2xl mx-auto">
            <div className="text-center"><div className="text-3xl font-bold text-amber-400">{personal.projects}</div><div className="text-sm text-gray-400">Projects</div></div>
            <div className="text-center"><div className="text-3xl font-bold text-amber-400">{personal.technologies}</div><div className="text-sm text-gray-400">Technologies</div></div>
            <div className="text-center"><div className="text-3xl font-bold text-amber-400">1</div><div className="text-sm text-gray-400">Certifications</div></div>
          </div>
        </motion.div>
        <motion.div animate={{ y: [0, 10, 0] }} transition={{ repeat: Infinity, duration: 1.5 }} className="absolute bottom-10 left-1/2 transform -translate-x-1/2 cursor-pointer" onClick={() => scrollToSection("about")}>
    ...
    <section id="about" ref={ref} className="relative py-20 overflow-hidden">
      <div className="max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5 }} className="text-center mb-12">
          <h2 className="text-4xl md:text-5xl font-bold inline-block bg-gradient-to-r from-amber-400 to-amber-600 bg-clip-text text-transparent">About Me</h2>
          <div className="w-24 h-1 bg-gradient-to-r from-amber-400 to-amber-600 mx-auto mt-4 rounded-full"></div>
        </motion.div>
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <motion.div initial={{ opacity: 0, x: -50 }} animate={isInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.6 }} className="space-y-6">
            <p className="text-gray-300 leading-relaxed text-lg">I'm <span className="text-amber-400 font-semibold">{personal.name}</span>, a passionate {personal.title} from {personal.location}.</p>
            <p className="text-gray-400 leading-relaxed">{personal.bio}</p>
            <div className="grid grid-cols-2 gap-4">
              <div className="flex items-center gap-3 text-gray-300"><FaCheckCircle className="text-amber-400" /><span>{personal.projects} Projects Completed</span></div>
              <div className="flex items-center gap-3 text-gray-300"><FaCheckCircle className="text-amber-400" /><span>{personal.technologies}+ Technologies</span></div>
              <div className="flex items-center gap-3 text-gray-300"><FaCheckCircle className="text-amber-400" /><span>1 Certification</span></div>
              <div className="flex items-center gap-3 text-gray-300"><FaCheckCircle className="text-amber-400" /><span>24/7 Support</span></div>
            </div>
          </motion.div>
          <motion.div initial={{ opacity: 0, x: 50 }} animate={isInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.6 }} className="relative">
            <div className="bg-gradient-to-br from-amber-500/10 to-transparent p-8 rounded-2xl border border-amber-500/30 backdrop-blur-sm">
              <h3 className="text-2xl font-bold text-amber-400 mb-6 flex items-center gap-2"><FaLaptopCode /> Developer Philosophy</h3>
              <div className="space-y-4">
                <div className="flex items-start gap-3"><FaHeart className="text-amber-400 mt-1" /><div><h4 className="font-semibold text-white">Passion for Code</h4><p className="text-gray-400 text-sm">Writing clean, maintainable, and scalable code is my priority.</p></div></div>
                <div className="flex items-start gap-3"><FaUsers className="text-amber-400 mt-1" /><div><h4 className="font-semibold text-white">User First</h4><p className="text-gray-400 text-sm">Creating intuitive interfaces that users love to interact with.</p></div></div>
                <div className="flex items-start gap-3"><FaRocket className="text-amber-400 mt-1" /><div><h4 className="font-semibold text-white">Continuous Growth</h4><p className="text-gray-400 text-sm">Always learning new technologies and best practices.</p></div></div>
              </div>
            </div>
          </motion.div>
        </div>
...
// Skills Section - FIXED with proper width handling
const SkillsSection = () => {
  ...
  return (
    <section id="skills" ref={ref} className="relative py-20 overflow-hidden">
      <div className="max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5 }} className="text-center mb-12">
          <h2 className="text-4xl md:text-5xl font-bold inline-block bg-gradient-to-r from-amber-400 to-amber-600 bg-clip-text text-transparent">Technical Skills</h2>
          <div className="w-24 h-1 bg-gradient-to-r from-amber-400 to-amber-600 mx-auto mt-4 rounded-full"></div>
        </motion.div>
...
const ExpertiseSection = () => {
  ...
  return (
    <section id="expertise" ref={ref} className="relative py-20 overflow-hidden bg-gradient-to-b from-transparent via-amber-500/5 to-transparent">
      <div className="max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5 }} className="text-center mb-12">
          <h2 className="text-4xl md:text-5xl font-bold inline-block bg-gradient-to-r from-amber-400 to-amber-600 bg-clip-text text-transparent">Core Expertise</h2>
          <div className="w-24 h-1 bg-gradient-to-r from-amber-400 to-amber-600 mx-auto mt-4 rounded-full"></div>
        </motion.div>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 w-full">
          {expertise.map((area, idx) => (
            ...
            >
              <div className={`absolute inset-0 bg-gradient-to-br ${area.color} opacity-0 group-hover:opacity-5 rounded-2xl transition-opacity`} />
              <div className={`inline-block p-3 rounded-xl bg-gradient-to-br ${area.color} bg-opacity-10 mb-4`}>
                <div className="text-amber-400 text-4xl group-hover:scale-110 transition-transform">{getIcon(area.icon)}</div>
              </div>
  ...
  return (
    <>
      <section id="projects" ref={ref} className="relative py-20 overflow-hidden">
        <div className="max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5 }} className="text-center mb-12">
            <h2 className="text-4xl md:text-5xl font-bold inline-block bg-gradient-to-r from-amber-400 to-amber-600 bg-clip-text text-transparent">Featured Projects</h2>
            <div className="w-24 h-1 bg-gradient-to-r from-amber-400 to-amber-600 mx-auto mt-4 rounded-full"></div>
          </motion.div>

          <div className="flex flex-wrap justify-center gap-3 mb-10">
            {categories.map((cat, idx) => (
              <motion.button key={idx} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => setFilter(cat)} className={`px-5 py-2 rounded-full text-sm font-medium transition-all ${filter === cat ? "bg-gradient-to-r from-amber-500 to-amber-600 text-black shadow-lg" : "border border-amber-500/30 text-amber-400 hover:bg-amber-500/10"}`}>
                {cat}
              </motion.button>
            ))}
          </div>
            ...
            {filteredProjects.map((project, idx) => (
              ...
              >
                ...
                <div className="p-5">
                  ...
                  <button onClick={() => setSelectedProject(project)} className="w-full py-2 border border-amber-500 rounded-lg text-amber-400 text-sm font-medium hover:bg-amber-500/10 transition-all flex items-center justify-center gap-2 group">
                    <FaEye /> View Details
                  </button>
                </div>
              </motion.div>
...
const JourneySection = () => {
  ...
  return (
    <section id="journey" ref={ref} className="relative py-20 overflow-hidden">
      <div className="max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5 }} className="text-center mb-12">
          <h2 className="text-4xl md:text-5xl font-bold inline-block bg-gradient-to-r from-amber-400 to-amber-600 bg-clip-text text-transparent">Development Journey</h2>
          <div className="w-24 h-1 bg-gradient-to-r from-amber-400 to-amber-600 mx-auto mt-4 rounded-full"></div>
        </motion.div>
        <div className="relative">
          ...
          <div className="space-y-8">
            {journey.map((milestone, idx) => (
              <motion.div key={idx} initial={{ opacity: 0, x: idx % 2 === 0 ? -50 : 50 }} animate={isInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.5, delay: idx * 0.1 }} className={`flex flex-col md:flex-row items-center gap-6 ${idx % 2 === 0 ? "md:flex-row" : "md:flex-row-reverse"}`}>
                <div className="md:w-1/2">
                  <div className={`bg-gradient-to-br from-amber-500/10 to-transparent rounded-2xl p-6 border border-amber-500/20 hover:border-amber-500/40 transition-all ${idx % 2 === 0 ? "md:mr-8" : "md:ml-8"}`}>
                    <div className="flex items-center gap-3 mb-3"><div className="text-amber-400 text-2xl">{getIcon(milestone.icon)}</div><span className="text-amber-400 font-bold text-lg">{milestone.year}</span></div>
                    <h3 className="text-xl font-semibold text-white mb-2">{milestone.title}</h3>
                    <p className="text-gray-400">{milestone.description}</p>
                  </div>
                </div>
                <div className="hidden md:block w-12 h-12 rounded-full bg-gradient-to-r from-amber-500 to-amber-600 flex items-center justify-center relative z-10 shadow-lg shadow-amber-500/50"><FaStar className="text-black text-sm" /></div>
                <div className="md:w-1/2" />
              </motion.div>
            ))}
          </div>
...
const EducationSection = () => {
  ...
  return (
    <section id="education" ref={ref} className="relative py-20 overflow-hidden bg-gradient-to-b from-transparent via-amber-500/5 to-transparent">
      <div className="max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5 }} className="text-center mb-12">
          <h2 className="text-4xl md:text-5xl font-bold inline-block bg-gradient-to-r from-amber-400 to-amber-600 bg-clip-text text-transparent">Education & Certifications</h2>
          <div className="w-24 h-1 bg-gradient-to-r from-amber-400 to-amber-600 mx-auto mt-4 rounded-full"></div>
        </motion.div>
        <div className="grid lg:grid-cols-2 gap-8 w-full">
          <motion.div initial={{ opacity: 0, x: -30 }} animate={isInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.5 }} className="bg-gradient-to-br from-amber-500/10 to-transparent rounded-2xl p-8 border border-amber-500/30 backdrop-blur-sm hover:shadow-xl transition-all">
...
const ContactSection = () => {
  ...
  return (
    <section id="contact" ref={ref} className="relative py-20 overflow-hidden">
      <div className="max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5 }} className="text-center mb-12">
          <h2 className="text-4xl md:text-5xl font-bold inline-block bg-gradient-to-r from-amber-400 to-amber-600 bg-clip-text text-transparent">Let's Connect</h2>
          <div className="w-24 h-1 bg-gradient-to-r from-amber-400 to-amber-600 mx-auto mt-4 rounded-full"></div>
        </motion.div>
        <div className="grid lg:grid-cols-2 gap-12 w-full">
          <motion.div initial={{ opacity: 0, x: -30 }} animate={isInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.5 }} className="space-y-6">
            <div className="bg-gradient-to-br from-amber-500/10 to-transparent rounded-2xl p-6 border border-amber-500/30">
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-4 text-gray-300 hover:text-amber-400 transition-colors group"><div className="w-10 h-10 bg-amber-500/20 rounded-full flex items-center justify-center text-amber-400 group-hover:scale-110 transition-transform"><FaEnvelope /></div><a href="mailto:meh67719@gmail.com" className="group-hover:text-amber-400">meh67719@gmail.com</a></div>
                <div className="flex items-center gap-4 text-gray-300 hover:text-amber-400 transition-colors group"><div className="w-10 h-10 bg-amber-500/20 rounded-full flex items-center justify-center text-amber-400 group-hover:scale-110 transition-transform"><FaPhoneAlt /></div><a href="tel:+8801747737704" className="group-hover:text-amber-400">+880 1747737704</a></div>
                <div className="flex items-center gap-4 text-gray-300 group"><div className="w-10 h-10 bg-amber-500/20 rounded-full flex items-center justify-center text-amber-400"><FaMapMarkerAlt /></div><span>Bangladesh</span></div>
                <div className="flex items-center gap-4 text-gray-300 hover:text-amber-400 transition-colors group"><div className="w-10 h-10 bg-amber-500/20 rounded-full flex items-center justify-center text-amber-400 group-hover:scale-110 transition-transform"><FaGlobe /></div><a href="http://www.mehedihassan.com" className="group-hover:text-amber-400">www.mehedihassan.com</a></div>
              </div>
            </div>
            <div className="bg-gradient-to-br from-amber-500/10 to-transparent rounded-2xl p-6 border border-amber-500/30">
              <h3 className="text-xl font-bold text-white mb-4">Follow Me</h3>
              <div className="flex gap-4">
                <motion.a whileHover={{ y: -3 }} href="https://github.com/mehedi67719" target="_blank" rel="noopener noreferrer" className="w-12 h-12 bg-amber-500/20 rounded-full flex items-center justify-center text-amber-400 hover:bg-amber-500 hover:text-black transition-all text-xl"><FaGithub /></motion.a>
                <motion.a whileHover={{ y: -3 }} href="https://linkedin.com/in/mehedihassan" target="_blank" rel="noopener noreferrer" className="w-12 h-12 bg-amber-500/20 rounded-full flex items-center justify-center text-amber-400 hover:bg-amber-500 hover:text-black transition-all text-xl"><FaLinkedin /></motion.a>
                <motion.a whileHover={{ y: -3 }} href="https://facebook.com/mehedihassan" target="_blank" rel="noopener noreferrer" className="w-12 h-12 bg-amber-500/20 rounded-full flex items-center justify-center text-amber-400 hover:bg-amber-500 hover:text-black transition-all text-xl"><FaFacebook /></motion.a>
              </div>
            </div>
          </motion.div>

          <motion.form initial={{ opacity: 0, x: 30 }} animate={isInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.5 }} className="space-y-4 bg-gradient-to-br from-amber-500/5 to-transparent rounded-2xl p-6 border border-amber-500/30" onSubmit={handleSubmit}>
